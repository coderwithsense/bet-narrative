...
import {
  ...
  const Marketplace = () => {
    const {nftCards} = useCreatorStories()
    return (
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {nftCards.map((item, index) => (
            <Card
              key={index}
              className="hover:border-r-4 hover:border-b-4 hover:border-green-500 p-4 rounded-lg transition-transform transform hover:scale-105 flex flex-col justify-between border"
            >
              <div>
                <img
                  src={item.coverImage}
              ...
              <div className="flex gap-5 ">
                <Button
                  onClick={() =>
                    (window.location.href = "/stories/view/AliceWonderland")
                  }
